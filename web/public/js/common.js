let common = {
    ...
    user_edit_update: (user_id = 0) => {
        ...
        let required_vars = [{'first_name': 'First name'}, {'last_name': 'Last name'}, {'phone': 'Phone'}, {'email': 'Email'},];
        let err_msg = '';

        for (const required_var of required_vars) {
            const [var_name, var_value] = Object.entries(required_var)[0]

            console.log(var_name, var_value)
            if (!data.hasOwnProperty(var_name) || !data[var_name]) {
                err_msg = 'Fill in the required field "' + var_value + '".';
                break;
            }
        }

        if (err_msg !== '') {
            html('user_modal_note', err_msg);
            remove_class('user_modal_note', 'fade');
            setTimeout(function() { add_class('user_modal_note', 'fade'); }, 3000);
            setTimeout(function() { html('user_modal_note', ''); }, 3500);
            return;
        }
